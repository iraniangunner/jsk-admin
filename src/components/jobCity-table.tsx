...
import { deleteJobCity, getJobCities } from "@/hooks/use-jobCity";
...
export function JobCityTable() {
  const [sorting, setSorting] = useState<SortingState>([
    { id: "created_at", desc: true },
  ]);

  const { data, isLoading, isError } = getJobCities();
  ...
  const columns: ColumnDef<JobCity>[] = [
...

    {
      ...
      cell: ({ row }) => {
        const jobCity = row.original;
        const [showDeleteDialog, setShowDeleteDialog] = useState(false);

        const { mutate: deleteJobCityById, isPending: isDeleting } =
          deleteJobCity();

        const handleDelete = () => {
          deleteJobCityById(jobCity.id, {
            onSuccess: () => {
        ...
        return (
          <div className="flex items-center justify-center gap-2">
            ...
            >
              <Link
                href={`/job-cities/${jobCity.id}/edit`}
                className="w-full h-full flex justify-center items-center"
              >
                <Edit className="h-4 w-4" />
                <span className="sr-only">ویرایش شهر</span>
              </Link>
            </Button>
            ...
            >
              ...
              <span className="sr-only">حذف شهر</span>
            </Button>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription className="sm:text-lg">
                    آیا از حذف این شهر اطمینان دارید؟
                  </DialogDescription>
                </DialogHeader>
      ...
      <div className="w-full" dir="rtl">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle>مدیریت شهرها</CardTitle>
              <CardDescription className="mt-2">
                لیست شهرهای مرتبط با فرصت های شغلی
              </CardDescription>
            </div>
            <Link href="/job-cities/create">
